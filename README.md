...
- SQLite FTS5 trigram 全文搜索（兼顾中文关键词匹配与搜索性能）
- 速率限制（按端点配置，支持 Redis 共享限流；默认内存后端）
...
- **文章点赞**：支持点赞/取消点赞（同一 IP 去重），按钮显示点赞数量，支持大数字格式化（k/w）
- 全站白天/黑夜主题切换（持久化）
...
- 移动端 hamburger 抽屉菜单
- 非首页折叠搜索（展开/收起动画）
- **搜索页优化**：SQLite FTS5 trigram 搜索、加载状态、空结果提示、关键词高亮、结果计数、卡片悬停动效、元信息 SVG 图标、移动端适配
...
- **媒体库**：Tab 设计（图片/音频/视频），文件上传自动提取图片分辨率，文件夹管理（创建/重命名/删除）、分页加载、拖拽排序
...
| DELETE | `/api/v1/admin/media/folders/{id}` | 删除目录 |
...
**性能与可扩展性**
- 新增可选 Redis 缓存后端,支持多 worker / 多实例共享缓存,连接失败时自动回退到进程内存缓存
- 文章浏览量改为异步队列批量写入,避免详情页请求同步执行去重查询和数据库更新
- 引入 SQLite FTS5 trigram 全文搜索索引与触发器,提升中文关键词搜索性能并保持索引自动同步
- 首页,搜索页和后台评论列表支持显式分页大小,后台评论筛选,排序,垃圾评论/垃圾箱状态改为服务端分页查询

**安全与健壮性**
- 上传图片新增最大尺寸与总像素限制,收紧图片格式校验并拒绝 SVG 上传,降低资源消耗和 XSS 风险
- 点赞接口改为可切换点赞状态,并校验文章必须已发布且未删除
- 管理后台仅在收到明确的 401 未授权响应时清除登录状态,网络或临时服务异常不再误判为登录失效
- RSS,Sitemap,robots.txt 支持 `PUBLIC_BASE_URL`,反向代理部署时可生成稳定的公开绝对地址

**前端体验与可维护性**
- 统一文章 slug / ID 链接生成逻辑,历史数据仍兼容 `/article/details/{id}`
- 搜索关键词高亮改为 Vue 安全渲染,避免将关键词直接拼接为 HTML
- 增加键盘 focus-visible 样式,触控目标尺寸和 `prefers-reduced-motion` 适配
- 新增文章工作流,媒体上传安全等自动化测试
...
- **页面过渡动画**：路由跳转时顶部渐变进度条 + 页面淡入淡出过渡，消除白屏闪烁
...
- 放宽速率限制：默认 300/60s、登录 10/60s、友链 60/60s、评论 20/60s
...
**后台（admin）**
- **媒体库文件夹管理**：新增文件夹创建/重命名/删除，文件可按文件夹组织管理，支持分页加载
- **站点条件加载**：登录状态检测后才加载站点设置，避免未登录时请求站点配置
- **仪表盘修复**：修复文章编辑时草稿内容异常持久化的问题

### 2026-07-20

**文档**
- 新增英文版 README（README.en.md），包含项目介绍、功能列表、API 参考、部署指南
- README.md 添加技术徽章（Python / Vue / FastAPI / SQLite / License）

**前台（web）**
- 分类页布局优化：卡片改为紧凑水平排列，减少页面滚动
...
**后端**
- 修复 Sitemap 文章 URL：`/article/details/{id}` → `/article/{slug}`（与前端路由一致，旧 ID 链接保留兼容）
