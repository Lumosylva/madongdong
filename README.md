...
- 后台友链管理列表改为更接近评论管理的表格式布局，顶部间距进一步收紧
...
当前仓库在前端构建前会执行 URL 风险扫描，用于提前拦截可能影响 HTTPS 上线的硬编码地址。
...
扫描规则由 `scripts/check-hardcoded-urls.config.json` 统一维护，分为两类：

- `patterns.allow`：允许白名单
  - 当前保留本地开发常用地址，例如 `http://127.0.0.1:8000`、`http://localhost:5173`、`http://localhost:5174`
- `patterns.deny`：禁止规则
  - `http://`
  - `ws://`
  - `localhost:`
...
如果扫描到不允许的 URL 写法，构建会直接失败，避免以下问题：

- HTTPS 页面加载 HTTP 资源导致混合内容拦截
- WebSocket 协议不匹配
- 生产环境误用本地地址
- 绝对域名在不同部署环境下失效
...
以后如果需要调整规则，只需要修改配置文件：

- 增加/删除扫描目录：修改 `scan.include`
- 排除新目录：修改 `scan.exclude`
- 增加允许项：修改 `patterns.allow`
